...
Q-But what to do if you want a variable declared inside case? 
    ...
    Q-Why cant we do int {x} 5; or int y{5}; ? but we can do it for default case where int u = 20 why is that ?
        ...
        declaration as it is just for compiler to know how much memory is to be reserved for this variables as declaration isnt a part of execution during runtime just compiler time) before first cases/default will 
        ...
        Also, compiler checks case by case meaning 1st. Compiler looks for first case or default in this example its Case 1 and sees there is a variable initialization above it that will get skipped if switch flow jumps to case 1 so it points out "crosses initialization error of int x", then after that it sees there is another initialization of int y{5}; which is fine for now
                                                   ...
                                                   5th. after that it sees there is no more cases after default so int u = 20; is fine as skipping initialization of variable u wont cause any problems as there are no cases after it.
